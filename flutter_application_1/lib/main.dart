...
class _VaultAppState extends State<VaultApp> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final schemeIndex = widget.store.themeIndex.clamp(0, pipSchemes.length - 1);
        final scheme = pipSchemes[schemeIndex];
        return AppPalette(
          scheme: scheme,
          schemeIndex: schemeIndex,
          onChangeScheme: (i) => widget.store.setTheme(i),
          child: MaterialApp(
            title: 'VAULT-O-MATIC',
            debugShowCheckedModeBanner: false,
            theme: buildRetroTheme(scheme),
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, _) {
          if (!widget.store.loaded) {
            return Scaffold(
              body: Center(
                child: Text(s.loading, style: retroStyle(size: 20, color: palette.accent)),
              ),
            );
          }
          ...
          final col         = over ? kRed : pct > 75 ? kYellow : palette.accent;
                      ...
                      children: [
                        Column(
                          children: [
                            ...
                            Text(
                              s.subtitle,
                              style: retroStyle(size: 11, color: palette.dark),
                            ),
                            const SizedBox(height: 2),
                            Text('VAULT-O-MATIC',
                                style: retroStyle(size: 36, color: palette.accent)),
                            Text(
                              '══════════════════════════════════',
                              style: retroStyle(size: 12, color: palette.dim),
                            ),
                          ],
                        ),
                        // Language toggle
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      border: Border.all(color: palette.dark, width: 1),
                                    ),
                                  ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(fmtMoney(store.balance),
                                      style: retroStyle(size: 28, color: palette.accent)),
                                ],
                              ),
                            ),
                            Column(
                        ...
                        return Expanded(
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: active ? palette.accent : kBg,
                                border: Border(
                                  bottom: BorderSide(
                                      color: active ? palette.accent : palette.dim,
                                      width: 2),
                                  right: BorderSide(
                                      color: palette.dim, width: 0.5),
                                ),
                              ),
                              child: Center(
                                child: Text(
                                  ...
                                  style: retroStyle(
                                    size: 15,
                                    color: active ? kBg : palette.accent,
                                  ),
                                ),
                              ),
                            ),
