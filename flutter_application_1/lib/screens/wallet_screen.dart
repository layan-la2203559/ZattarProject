      ...
      children: [
        ...
        RetroBox(
          ...
          children: txs.isEmpty
              ? [
                  Text(s.noRecords,
                      style: retroStyle(color: palette.dim, size: 16))
                ]
              : txs.take(50).map((tx) {
                  final isAdd = tx.type == 'ADD';
                  final col   = isAdd ? palette.accent : kRed;
                  final sign  = isAdd ? '+' : '-';
                  // Subtle tinted text for notes
                  final noteCol = Color.lerp(palette.accent, Colors.white, 0.4)!;
                  return Column(
                    children: [
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(tx.note,
                                  style: retroStyle(size: 13, color: noteCol),
                                  overflow: TextOverflow.ellipsis),
                            ),
                            Text(tx.date,
                                style: retroStyle(size: 11, color: palette.dim)),
                      ...
                      Container(height: 1, color: palette.dim),
                    ],
                  );
                }).toList(),
        ),
